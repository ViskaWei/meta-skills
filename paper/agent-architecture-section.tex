...
Complex scientific workflows demand agents that can not only execute multi-step tasks but also learn from their own execution.
A static collection of capabilities---however well-organized---cannot anticipate every failure mode, adapt to new domains, or close quality gaps discovered only at runtime.
...
The layered architecture enables compositional reuse while presenting users with a simple interface.
The evolution loop systematically feeds execution observations back into the system's own capabilities, quality policies, and accumulated knowledge.
...
Two foundational ideas shape the architecture and explain why the system can grow without destabilizing.
...
Together, these two principles---separation by change rate and mechanical enforcement---create a foundation stable enough to support continuous modification, which the next subsection exploits.
