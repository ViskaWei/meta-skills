...
\node[cmdbox] at (-2.5, 0) {/meta};
\node[cmdbox] at (0, 0) {/build};
\node[cmdbox] at (2.5, 0) {/research};
\node[sublbl, anchor=west] at ([xshift=4pt]l0.east) {\textit{entry cmds}};

% Visibility annotation (symmetric, anchored to band edges)
\coordinate (visR) at ([xshift=2pt, yshift=-0.35cm]l0.south east);
\coordinate (visL) at ([xshift=-2pt, yshift=-0.35cm]l0.south west);
...
\node[pathbox] at (0, -2.2) {hypothesis-to-evidence};
\node[pathbox] at (3.5, -2.2) {improve-loop};
\node[sublbl, anchor=west] at ([xshift=4pt]l1.east) {\textit{path templates}};
...
\node[stagebox=knowledgegold] at (5.25, -4.55) {Knowledge};
\node[sublbl] at (0, -5.45) {29 contract-driven atomic capabilities with typed inputs / outputs};
\node[sublbl, anchor=west] at ([xshift=4pt]l2.east) {\textit{capabilities}};

% === ARROWS L0 -> L1 ===
\draw[arrw] (0, -0.75) -- (0, -1.45);
\node[sublbl, anchor=west] at (0.12, -1.25) {\textit{keyword match}};

% === ARROWS L1 -> L2 ===
\draw[arrw] (0, -2.95) -- (0, -3.9);
\node[sublbl, anchor=west] at (0.12, -3.4) {\textit{resolve \& expand}};

% === QUALITY POLICIES (bottom strip, symmetric via band anchors) ===
\draw[policyred, line width=1pt, dashed, rounded corners=4pt]
    ([xshift=-2pt, yshift=-0.3cm]l2.south west) rectangle ([xshift=2pt, yshift=-0.8cm]l2.south east);
...
\begin{tikzpicture}[
    >=Stealth,
    % --- Loop node styles (2 colors: action vs analysis) ---
    actnode/.style={  % Left column: Execute + Evolve (action/doing)
        rounded corners=4pt, draw=discoverblue!60, fill=discoverblue!7,
        minimum width=2.6cm, minimum height=0.85cm,
        font=\small\sffamily, align=center, line width=0.9pt},
    monnode/.style={  % Right column: Observe + Review (monitoring/evaluating)
        rounded corners=4pt, draw=reviewteal!60, fill=reviewteal!7,
        minimum width=2.6cm, minimum height=0.85cm,
        font=\small\sffamily, align=center, line width=0.9pt},
    % --- Mechanism box style (parametric color) ---
    mechbox/.style={
        rounded corners=3pt, draw=#1!50, fill=#1!6,
        minimum height=0.55cm, inner sep=5pt,
    ...
    feedbackedge/.style={->, black!35, line width=0.7pt, dotted,
        shorten >=1pt, shorten <=1pt},
    crossedge/.style={->, black!25, line width=0.5pt, dashed},
]

% === MAIN LOOP (4 nodes, clockwise rectangle) ===
% Left column (blue) = action; Right column (teal) = analysis
\node[actnode] (exec) {Execute};
\node[monnode, right=2.8cm of exec] (observe) {Observe};
\node[monnode, below=1.8cm of observe] (review) {Review};
\node[actnode, below=1.8cm of exec] (evolve) {Evolve};

% Subtle background grouping
\begin{scope}[on background layer]
    \node[fit=(exec)(observe)(review)(evolve),
        rounded corners=8pt, fill=black!2, draw=black!8, line width=0.3pt,
        inner sep=15pt] {};
\end{scope}

% Clockwise primary arrows
\draw[primary] (exec) -- node[above, annot] {6 signal types} (observe);
\draw[primary] (observe) -- (review);
\draw[primary] (review) -- node[below, annot] {approve} (evolve);

% Feedback: Evolve -> Execute (left channel, outside the rectangle)
\draw[feedbackedge] (evolve.west) -- ++(-0.9, 0) |- (exec.west);

% Dual-agent annotations
\node[annot, above=0.3cm of exec] {executor (Sonnet)};
\node[annot, above=0.3cm of observe] {observer (Haiku)};

% Evolve output annotation (centered below the loop)
\node[annot] at ($(evolve.south east)!0.5!(review.south west) + (0, -0.3)$)
    {$\rightarrow$ new L2 cap ~/~ L1 path ~/~ rule-*};

% === SUPPORTING MECHANISMS (aligned below loop) ===
\node[mechbox=policyred] (pol) at ($(evolve.south) + (0, -2.2)$)
    {Policy Ratchet};
\node[mechbox=memoryblue] (mem) at ($(review.south) + (0, -2.2)$)
    {Three-Layer Memory};

% Mechanism sub-labels
\node[annot, anchor=north] at ([yshift=-0.08cm]pol.south)
    {failure $\rightarrow$ rule $\rightarrow$ enforce};
\node[annot, anchor=north] at ([yshift=-0.08cm]mem.south)
    {L1 auto $\cdot$ L2 handoff $\cdot$ L3 archive};

% Cross-links from loop to mechanisms
\draw[crossedge] (evolve) -- (pol);
\draw[crossedge] (review) -- (mem);

\end{tikzpicture}
\caption{Continuous self-evolution loop.  Every skill execution spawns a dual-agent team: an executor performs the task while an observer detects improvement signals.  Findings are reviewed and, once approved, fed back as new capabilities, paths, or policies.  A policy ratchet mechanically enforces every codified failure mode; a three-layer memory stack ensures cross-session continuity.}
